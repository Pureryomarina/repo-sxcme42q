...
bool AndroidImgui::Init_Render(ANativeWindow *window, float width, float height) {
    ...
    style.FrameBorderSize = 0.3f; // 设置控件描边宽度
...
void AndroidImgui::RecreateWindow(ANativeWindow *window, float width, float height) {
    // 清理旧的 Vulkan 资源，但保持 ImGui 上下文
    PrepareShutdown();
