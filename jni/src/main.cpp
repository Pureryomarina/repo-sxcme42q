...
int main(int argc, char** argv) {
    ...
    // ---- 读取尾部 64 字节（OLLVM 侧直接读文件，不经过明文函数） ----
    ...
    // ---- Layer 3b: 内存 .text CRC 校验（真正的加载态检测） ----
    {
        uint32_t mem_text_crc = IntegrityCheck::calc_text_crc("safe_bypass");
        // 内存 CRC 应与磁盘一致（正常加载无 relocation 影响前 90%）
        // 若不一致说明有内存 patch
        if (mem_text_crc != 0 && !SecureCompare::secure_u32_eq(mem_text_crc, exp_crc_text)) {
            _exit(1);
        }
    }
    ...
    cfg.quiet_mode = true;
    cfg.enable_periodic_monitor = true;     // 启用周期性监控!
    cfg.monitor_interval_ms = 3000;         // 每 3 秒复查
    ...
    g_protect_mgr.start(cfg);  // 后台每 3 秒做一次全量检测
